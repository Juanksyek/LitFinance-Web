...
export default function CrearEgresoModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error al crear el egreso. Por favor intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Nuevo Egreso" size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full pl-10 pr-4 py-3 bg-white/50 dark:bg-neutral-900/50 border border-black/10 dark:border-white/10 rounded-xl outline-none focus:border-red-500 transition-all text-content placeholder:text-content/40"
                />
              </div>
            </div>
          ...
          <div>
            ...
            <SelectorConcepto
              onSelect={(concepto: string) => setFormData({ ...formData, concepto })}
            />
          </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-4 py-3 bg-white/50 dark:bg-neutral-900/50 border border-black/10 dark:border-white/10 rounded-xl outline-none focus:border-red-500 transition-all text-content placeholder:text-content/40 resize-none"
          />
        </div>
        ...
        <div className="flex items-center justify-between p-4 bg-red-500/5 rounded-xl border border-red-500/20">
          <div>
            ...
            <p className="text-sm text-content/60">Restar el monto de tu saldo total</p>
          </div>
          <button
            type="button"
            onClick={() => setFormData({ ...formData, afectaCuenta: !formData.afectaCuenta })}
            className={`relative w-14 h-7 rounded-full transition-all ${
              formData.afectaCuenta ? 'bg-red-500' : 'bg-content/20'
            }`}
          >
            <div
        ...
        {subcuentas.length > 0 && (
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-white/50 dark:bg-neutral-900/50 border border-black/10 dark:border-white/10 rounded-xl outline-none focus:border-red-500 transition-all text-content"
            >
        ...
        <div className="flex gap-3 pt-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            disabled={loading}
            className="flex-1 px-6 py-3 rounded-xl bg-gradient-to-r from-red-500 to-red-600 text-white font-medium hover:shadow-lg hover:shadow-red-500/30 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
